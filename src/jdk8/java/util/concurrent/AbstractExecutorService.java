        ...
        try {
            ...
            for (;;) {
                ...
                if (f == null) {
                    ...
                    if (ntasks > 0) {
                    ...
                    else if (active == 0)  //已经没有正在执行的任务了
                        break;
                    else if (timed) {  //如果启用了超时设置
                        //取出已结束的任务，取不到时阻塞一段时间
                        ...
                        f = ecs.take();  //取出一个已结束的任务，取不到时陷入阻塞
                }
                //如果从阻塞队列中取到了任一已结束的任务
                if (f != null) {
                    //正在执行的任务数减一
